...
export const freightModules: FreightModule[] = [
  ...
  createModule({
    ...
    fields: [
      f('name', 'Role', 'តួនាទី', 'Main information', 'ព័ត៌មានទូទៅ', 'text', undefined, { required: true }),
  ...
  createModule({
    ...
    columns: [
      ...
      col('user', 'User Name', 'ឈ្មោះអ្នកប្រើ'),
      col('ipAddress', 'Request IP', 'IP សំណើ'),
    ],
    fields: [
      f('occurredAt', 'Time', 'ពេលវេលា', 'Log', 'កំណត់ហេតុ', 'datetime'),
      f('action', 'Action', 'សកម្មភាព', 'Log', 'កំណត់ហេតុ'),
      f('result', 'Result', 'លទ្ធផល', 'Log', 'កំណត់ហេតុ'),
      f('user', 'User Name', 'ឈ្មោះអ្នកប្រើ', 'Log', 'កំណត់ហេតុ'),
      f('ipAddress', 'Request IP', 'IP សំណើ', 'Log', 'កំណត់ហេតុ'),
    ],
    filters: [
...
if (quotationModule) freightModules.push({
  ...
  columns: [
    col('quotationNo', 'Quotation No.', 'លេខសម្រង់'), col('customer', 'Customer', 'អតិថិជន'),
  ...
  fields: [
    ...
    f('currency', 'Currency', 'រូបិយប័ណ្ណ', 'Header', 'ក្បាល', 'select', CURRENCIES, { required: true }), f('description', 'Description', 'បរិយាយ', 'Header', 'ក្បាល', 'textarea', undefined, { colSpan: 2 }),
    f('status', 'Status', 'ស្ថានភាព', 'Header', 'ក្បាល', 'select', QUOTATION_STATUS),
  ...
  filters: [f('customer', 'Customer', 'អតិថិជន', '', '', 'select'), f('direction', 'Trade Directions', 'ទិសដៅពាណិជ្ជកម្ម', '', '', 'select', DIRECTIONS, { labelKey: 'freight.ui.cols.direction' })],
})
...
if (jobsModule) freightModules.push({
  ...
  description: 'Operational orders, containers, components, charges, documents and audit history.',
  descriptionKm: 'បញ្ជាប្រតិបត្តិការ កុងតឺន័រ សមាសភាគ ថ្លៃ ឯកសារ និងប្រវត្តិសវនកម្ម។',
  columns: [
...
if (chargesModule) freightModules.push({
  ...
  filters: [f('jobNo', 'Service Order', 'បញ្ជាសេវាកម្ម'), f('customer', 'Customer', 'អតិថិជន'), f('documentType', 'Document Type', 'ប្រភេទឯកសារ', '', '', 'select', ['SERVICE_NOTE', 'DEBIT_NOTE', 'PRO_FORMA']), f('status', 'Status', 'ស្ថានភាព', '', '', 'select', SERVICE_CHARGE_STATUS)],
})
...
if (invoicesModule) freightModules.push({
  ...
  fields: [
    ...
    f('customer', 'Party', 'ដៃគូ', 'Party & Scope', 'ដៃគូ និងវិសាលភាព', 'text', undefined, { required: true }), f('jobNo', 'Service Order', 'បញ្ជាសេវាកម្ម', 'Party & Scope', 'ដៃគូ និងវិសាលភាព'),
  ...
  filters: [
    f('documentType', 'Document Type', 'ប្រភេទឯកសារ', '', '', 'select', ['CUSTOMER_INVOICE', 'SUPPLIER_BILL', 'CUSTOMER_RECEIPT', 'SUPPLIER_PAYMENT', 'OTHER_INCOME', 'OTHER_EXPENSE', 'TRANSFER', 'ADJUSTMENT']), f('customer', 'Party', 'ដៃគូ', '', '', 'select'), f('jobNo', 'Service Order', 'បញ្ជាសេវាកម្ម', '', '', 'select'), f('status', 'Status', 'ស្ថានភាព', '', '', 'select', DEBIT_NOTE_STATUS), f('currency', 'Currency', 'រូបិយប័ណ្ណ', '', '', 'select', CURRENCIES),
  ],
  tables: [
    {
    ...
    { key: 'sourceRelationships', title: 'Source Relationships', titleKm: 'ទំនាក់ទំនងប្រភព', columns: SOURCE_RELATIONSHIP_COLUMNS },
    { key: 'journalEntries', title: 'Journal', titleKm: 'ទិនានុប្បវត្តិ', columns: [{ key: 'account', label: 'Account', labelKm: 'គណនី', required: true }, { key: 'description', label: 'Description', labelKm: 'បរិយាយ' }, { key: 'debit', label: 'Debit', labelKm: 'ឥណពន្ធ', type: 'number' }, { key: 'credit', label: 'Credit', labelKm: 'ឥណទាន', type: 'number' }] },
    { key: 'attachments', title: 'Files', titleKm: 'ឯកសារ', addLabel: 'Upload File', addLabelKey: 'freight.ui.uploadFile', kind: 'files', columns: FILE_ATTACHMENT_COLUMNS },
