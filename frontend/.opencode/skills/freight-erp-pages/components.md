...
- Clone a page shell into a per-page list component (e.g. a second service-order list). Extend the module schema instead and delete the clone.
- Write inline badge-color ternaries. Build cells through the shared status-badge helper (`statusColor()` + `UBadge` `variant="subtle"` `size="sm"`), like `formatFreightCell`.
- Copy date/format helpers (`shortDay`, …) between files. Centralize them beside `formatFreightCell` in the freight composables.
- Use native `window.prompt` / `alert` / `confirm`. Confirm with `AppConfirmDialog` / `useConfirm`; collect a reason with a small `UModal` form (reverse-reason modal pattern).
- Hand-roll raw `<table>` markup. Use `UTable` plus the project's compact utilities.
- Leave orphaned or superseded `.vue` components in the tree. Delete them.
...
## Known debt register (2026-08 reusability audit)

Fix on sight when touching these files; remove entries as they are completed.

- [x] Delete orphaned legacy components (0 references): `freight/JobCustoms.vue`, `JobBooking.vue`, `JobTracking.vue`, `Checklist.vue`, `ProgressSteps.vue`, `JobProfit.vue`.
- [x] Remove `freight/ServiceOrderList.vue` — extend the service-orders schema in `freight-modules.ts` and render via `WorkspaceView`.
- [x] Replace hand-built field grids with `FieldGrid` → `FieldInput` (size `md`): `JobCharges.vue`, `JobContainers.vue`, `JobFiles.vue`, `JobRoute.vue`; render `JobTasks.vue` template attributes through the dynamic field renderer.
- [ ] Replace the remaining hand-built field grid in `common/AppUserProfileDialog.vue` with `FieldGrid` → `FieldInput`.
- [x] `DocumentView.vue`: posting-preview grid → i18n definition list (`postingPreviewItems`); `window.prompt` reverse reason → `UModal` + `FINANCE_REVERSE_FORM_FIELDS` via `FieldGrid`.
- [x] Options & helpers back into config/utils: `DOCUMENT_TYPES` (+ Supporting Document) for job files, `CONTAINER_STATUSES`, report status filters from `JOB_WORKFLOW_STATUS` / `CONTAINER_STATUSES`, Telegram destination types in `select-options.ts`, `shortDay` + `freightStatusBadge` beside `formatFreightCell`.
- [ ] Move the 12-report catalog out of `ReportsView.vue` into a config file (Phase 5).
- [ ] Convert remaining raw `<table>` blocks (`JobContainers.vue`, `JobRoute.vue`) to `UTable` with the compact utilities.
